...
日常更新分三步。第一步把 `kustomization.yaml` 里三个业务镜像的标签改为新的完整 SHA（直接编辑 newTag，或用 `kustomize edit set image`）。第二步删除上一次的迁移任务：Job 的模板不可修改，镜像标签变了以后直接 apply 会报错，所以每次更新都要先执行 `kubectl -n combo delete job migrate --ignore-not-found`。第三步执行 `kubectl apply -k infra/k8s`，并用 `kubectl -n combo wait --for=condition=complete job/migrate --timeout=300s` 确认迁移成功，业务副本会按滚动策略自行更新。

Web 服务通过节点的 `30080` 端口对外提供服务。观测栈与流量切换不在本套清单内。
