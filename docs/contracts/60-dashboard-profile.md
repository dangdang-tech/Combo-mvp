...
>
> **真源**：技术架构以飞书《技术方案 · 创作者中心与消费链路》为权威，本地落地以 `docs/01-详细技术方案.md` 为准（B-32/B-33/B-34、数据模型 §4、差异 §9 第 7/9 条）；产品行为以 `docs/开工总纲-创作者中心主链路.md` 为准；验收口径以 `docs/测试验收-创作者中心主链路.md`（外壳首页- / 主页- 两模块）为准。
>
...
export interface DashboardCapabilityRow {
  ...
  retryEditable: boolean; // review_rejected → true，供创作流程判断能否基于被拒版恢复；不派生行内编辑入口
  ...
  // 唯一行内动作真源：true 才展示「公开页」，false 时操作列不为未来能力造占位入口。
...
>
> **行内动作收敛**（外壳首页-14/15/35、F-05）：能力行只在 `publicPageAvailable=true` 时展示「公开页」，并导航到 `/a/{slug}`。`false` 时不展示公开页，也不展示试用、编辑、下架、改价或「更多」等未开放动作。创建新 Agent 只保留在页面级 CTA，不在每一行重复。
>
> **拒绝态可见**（B-30、外壳首页-11 行内、F-05 status 列）：`reviewStatus='review_rejected'` 时 `statusLabel="已退回"`、`rejectReason` 给简单人话原因、`retryEditable=true`。这是 B-30「三处同步可见」的工作台落点；`retryEditable` 不等于行内编辑按钮。
>
> **数据源**：`capabilities` + `capability_versions`（name/tagline/status/visibility）+ `publications`（review_status/reject_reason）。被拒回退时 `versionId` 指 `capabilities.current_version_id`（已回退到上一 published），状态展示回退后口径。`publicPageAvailable` 必须与公开 Runtime 的真实可达条件同源，不能只凭「历史上存在 published 版本」推断。
>
> 验收对齐：外壳首页-11（列项齐全、usage 子断言占位）、外壳首页-14（公开页仅在真实可达时出现）、外壳首页-15（创建新 Agent 仅页面级 CTA）、外壳首页-35（不展示未开放动作或「更多」占位）、外壳首页-30/B-30（拒绝态徽章+原因可见）。
...
export interface DashboardCapabilityRow {
  ...
  monthlyInvocations: number | null;
  spendSparkline: TrendPoint[] | null;
...
export interface ProfileMetricsBand {
  ...
  hottestTopic: { name: string | null; heatValue: number | null };
  readonly: true;
}
export interface DensityRankRow {
  rank: number;
...
export interface CreatorProfile {
  creatorId: UserId;
  slug: Slug;
  sectionsOrder: ProfileSectionKey[];
  hero: ProfileHero;
...
| 模块                    | 用例编号（本域覆盖）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | 本期口径                                                                                                                                                                                                                                                                    |
| ----------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **外壳首页-**（工作台） | 08（摘要）、09（四卡环比）、10（趋势双口径）、11（能力表列项）、14（公开页条件动作）、15（页面级创建 CTA）、16/17/18（草稿条/续传）、19（时间范围三档）、20（只对本人可见）、21（钱不外泄）、22（新账号空态）、23（无草稿空态）、24（加载占位不转圈）、25（失败带退路）、26（无消耗空态）、29（大数字环比）、30（续传不丢）、32（活跃消费者含匿名）、33/34（草稿条进度/多条不串台）、35（不展示未开放动作）                                                                                                                                                                               | usage 类（08/09/10/11/19/20/26/29/32）按占位「暂无数据/上线后填充」判过；14 只在 `publicPageAvailable=true` 时展示「公开页」；15 创建新 Agent 仅页面级 CTA；35 不以禁用项、占位反馈或「更多」暴露未开放动作；30/33/34 续传按 P0 真实验；20/21/25 只读/退路/不裸码照常 P0 验 |
...
### 7.3 被改判 / 挂起的验收用例（本域 usage 占位与动作真实性口径）
...
| **能力行动作收敛**          | 外壳首页-14（公开页条件动作）、-15（创建入口）、-35（扩展动作）                                                                                                                                                                                                                                                                                                                  | `publicPageAvailable=true` 才展示行内「公开页」；创建新 Agent 仅页面级 CTA；试用、编辑、下架、改价与「更多」等未开放动作不渲染，不以 disabled 或「本期未开放」占位。公开页可见时必须真实可达，不得落 404                                               |
