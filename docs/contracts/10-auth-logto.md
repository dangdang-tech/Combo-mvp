...
>
> **依赖脊柱**：本文严格遵循 `docs/contracts/00-约定与状态机.md`（下称「脊柱」）。路由前缀 `/api/v1`、响应包络 `Envelope<T>` / `Paginated<T>`、错误信封 `ErrorEnvelope` + action 五枚举、幂等 `Idempotency-Key` + 行为矩阵、SSE 帧协议、共享 TS 类型（§9）、健康检查口径（§10）一律 **import 不重定义**。
>
> **真源**：技术架构以飞书《技术方案 · 创作者中心与消费链路》为权威，本地以 `docs/01-详细技术方案.md` B-08 / §6.1 / §6.2 / §6.3 为准；产品角色口径以 `docs/开工总纲-创作者中心主链路.md` §1.3 为准；验收口径以 `docs/测试验收-创作者中心主链路.md` 外壳首页-01/20/31/32、主页-13 为准。
>
