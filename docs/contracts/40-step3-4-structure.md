...
> 2. **绝不裸露错误码** —— 单**软**字段两次重试仍失败落 `ErrorEnvelope`（脊柱 §11.B + D1：UI 唯一可展示 = `userMessage` 人话 + `action` 三选一 retry/change_input/escalate；**对外信封不含 code**，内部 `code=STRUCTURE_FIELD_FAILED` 仅日志/告警/映射、经 traceId 关联）；SSE `error` 帧 = 完整对外 `ErrorEnvelope`（Codex#2）；硬字段锁定不参与生成、不报字段级错误（`details.field` 恒 SoftFieldKey）。落于 §3.4、§4 各端点错误用例。
...
- 选择结果暂存在前端向导状态，在进入下一步（结构化 / 批量发布）时随该步请求体发出：
  - 逐个选（`single`）→ 选中的 `candidateId` 作为 **STEP④ 入口端点 A**（`POST /capabilities`）的 `sourceCandidateId`（验收 选择结构化-07：右侧当前能力名称对得上）。
  - 勾选 N 项 / 全部发布（`subset`）→ 选中的 `candidateIds[]`（N<total 或 N==total 都可）进入**发布域批量发布**（`POST /publish-batches`，每 candidate 一个 item，见发布域契约 B-29；本域不覆盖）。「全部发布」= subset==全 ready 的特例。

**(b) 保存草稿 / 进入下一步 = 持久化 selection（PRD「每步可存草稿」落地）**：

- 顶栏「保存草稿」按钮 → 调本域**端点 G**（`PATCH /api/v1/drafts/{draftId}/selection`，§4.G）把当前选择**显式持久化** `drafts.selection`（`single{candidateId}` 或 `subset{candidateIds}`，含兼容别名 `all`，§4.G schema）+ `drafts.current_step='select'`，使工作台草稿条出现「选择中」、可断点续传回精确选择态。这是**用户显式动作或自动节流保存**，不是「每次点选写库」——避免选择步退化成有加载态的写操作。
...
| A   | `POST /api/v1/capabilities`                                            | 建能力体 draft 版本——三分支**恰好三选一**（Codex#7，`sourceCandidateId`/`capabilityId`/`fromVersionId` 有且仅有一个）：①从候选新建首版 ②published 后建新版本（`capabilityId`）③被拒重发派生新 draft（`fromVersionId`，复制被拒版 manifest 软字段） | B-24/B-26          |
...
export type HardFieldKey = 'id' | 'version' | 'status' | 'inputs' | 'output' | 'boundaries';
...
对同一**软字段**（结构化 Job 内部重试 ≤2，或用户经端点 F `regen` 累计）**重试两次仍失败** → 该字段落终态错误，发 `error` 帧（**完整对外 `ErrorEnvelope`**，线上帧外形 `data: {"error": {...}}`，脊柱 §3.1/§5.3/Codex#2；**对外不含 code**，D1），`error.action` 三选一（`retry` / `change_input` / `escalate`，对齐 Figma「重试 / 改输入 / 转人工」）。内部 code `STRUCTURE_FIELD_FAILED` 只进日志、经 `traceId` 关联，**不进帧 payload**（D1）。**字段级失败只可能发生在 7 个软字段（`error.details.field` ∈ `SoftFieldKey`）**；硬字段锁定不生成、永不落字段级失败（验收 选择结构化-11/27）：

```
id: 1718460040-0
event: error
data: {"error":{"userMessage":"这个字段没生成出来，可重试、改输入或转人工。","retriable":true,"action":"escalate","traceId":"01J...","details":{"field":"instructions","attempts":2}}}
```

- 线上帧外形是**完整对外 `ErrorEnvelope`**（`data: {"error": {...}}`，与 import/job 流 `error` 帧外形一致，脊柱 §3.1/§5.3/Codex#2）；内层 `error` 遵脊柱 §11.B + D1：UI 唯一可展示 = `error.userMessage`（人话）+ `error.action`；**对外帧不含 `code`**（D1）——内部 code `STRUCTURE_FIELD_FAILED` 仅日志/告警/文案映射、经 `traceId` 关联；`error.traceId` 可作「反馈代码」展示。`error.details.field` ∈ `SoftFieldKey`。**DB 持久化**（`capability_versions.structure_state[field].error`）存的也是无 code 的内层 `ErrorEnvelope['error']`（与对外形态一致）；SSE 线上帧是完整对外信封。
...
- **累计失败次数靠 `structure_state[field].attempts` 持久化**（`FieldState.attempts`，脊柱 §9；缺省视作 0），夹在 `[0, LLM_MAX_RETRIES]`（不溢出）：「同一软字段重试两次」是**跨 Job、跨端点 F `regen` 调用累计**的——
  - **full 自动结构化**：本 Job 内部重试 ≤ `LLM_MAX_RETRIES=2`；同一 Job 内两次仍失败即落 `escalate` 终态。
  - **端点 F 单字段 `regen`**：**每次点击 = 一次用户驱动尝试**（本轮预算 1，**不是每次 regen 都重置为全新 2 次**，否则 §3.4 线上永不触发）。第一次点击失败 → `attempts=1`、发 `error` 帧但 `action='retry'`（给重试退路、不裸转圈，未到终态）；用户再点 `regen` → `attempts` 累计达 2 → 落 `action='escalate'` 终态（转人工）。
  - 端点 F 受理（`markFieldGenerating`）回读该字段已存 `attempts`（不清零）作 `attemptsBefore` 透传进 `subject_ref`；worker 据此起算，每次失败把累计 `attempts` 写回 `structure_state`（受保护写 CTE，对父行单语句一次落，§11.A）。
  - **字段成功生成 / 手动 PATCH 该字段则 `attempts` 重置 0**（下次干净起算）；改其它软字段不连坐本字段的 `failed`/`attempts`（其它字段累计不丢，§3.4）。

### 3.5 断线重连续传（脊柱 §5.4，验收 选择结构化-22、贯穿-10/28）

- 每帧 `id:` = Redis Stream entry id；客户端 `Last-Event-ID` 重连。事件源 = 该 version 的 active structure job 流（`events:job:{jobId}`，worker 经 `ctx.emitField` 写）；SSE 端点 D 把它可靠映射到 active job 并 live subscribe（同 3A job 流：latestId 锚点 → snapshot → 从锚点订阅，消 TOCTOU）。
- 窗内 → 从该 id 后补增量（不重推 snapshot）；超窗 / 无 Last-Event-ID → 先推 `state_snapshot(structure)` 重置再续。统一终态闸：active job 非 running（已终态 / 无 job）→ snapshot 后补 `done` 关流、**绝不 subscribe**（无悬挂心跳、无双 done）。
- 续传精度靠 `structure_state`：**已生成软字段原样回显、只补未生成字段、不打回加载条**（贯穿-28）。**`stuck` 态持久化**：某软字段触发 `field_stuck` 时受保护写 `structure_state[field].status='stuck'` + `stuckMs`，断线重连 `state_snapshot` 能重建三退路状态（完成/失败再转 `done`/`failed`，stuck 清）。**`failed` 态含 `error`**（`FieldState.error`，无 code），重连回显错误态 + 退路。
...
>
> 写命令（POST/PATCH）必带 `Idempotency-Key`（脊柱 §4）。响应成功裹 `Envelope<T>`，失败只出 `ErrorEnvelope`（脊柱 §11.B + D1：UI 唯一可展示 = `userMessage` 人话 + `action`，**对外信封不含 code**，内部 code 仅日志/告警/文案映射、经 traceId 关联）。下文各端点错误用例表「userMessage（人话）」列即 `userMessage`，「code」列是**内部 code（仅日志/映射，不进对外 payload）**，action/retriable 遵脊柱 §3.3 缺省。
...
```typescript
// SelectionDraft = drafts.selection 的权威形态（持久化 + 续传恢复 schema；子集化 P0-1，§5.2/§5.3）。
//   · single（candidateId）—— 逐个选定一个候选。
//   · subset（candidateIds: [≥1]）—— 勾选 N 项（N<total 或 N==total 都合法，§5.2 批量勾选 N 项）。
//     「全部发布」不是独立模式，只是 subset==该 snapshot 全部 ready 候选的特例（§5.3）。
//   兼容/迁移：保留 'all' 为 subset 的【向后兼容别名】（语义等同 subset：⊆ ready、非空），
//     使旧持久化草稿续传、未迁移前端继续编译；新写一律用 subset。
export const SelectionDraft = z.discriminatedUnion('mode', [
  z.object({
    mode: z.literal('single'),
    candidateId: z.string().uuid(), // 逐个选：选中的单个候选
  }),
  z.object({
    mode: z.literal('subset'),
    candidateIds: z.array(z.string().uuid()).min(1), // 勾选 N 项（非空；N 可 < total，子集合法）
  }),
  z.object({
    mode: z.literal('all'), // 向后兼容别名（= subset；旧草稿/未迁移前端续命，新写用 subset）
...
- 校验 `draftId` 属当前 `ctx.userId`（非 owner → `403`）；校验 `selection` 内候选属同一来源快照 / 属本人 / 都是 ready（非法候选 → `404`/`400`）。
- **子集校验（P0-1，§5.2/§5.3）**：`candidateIds`（single 视作 1 个）须 **⊆ 本人该 snapshot 的 ready 候选集** 且非空、无重复；**不要求 == 全 ready**——勾选任意子集（N<total）合法（§5.2 批量勾选 N 项），「全部发布」只是 subset==全 ready 的特例（§5.3）。含他人/跨快照/非 ready/重复 → `400 VALIDATION_FAILED`（人话，无 code）。
...
- 续传：工作台续传读 `DraftView.currentStep='select'` + `DraftView.selection`（= `SelectionDraft`）→ 回 STEP③/STEP② 预置选择态（恢复 schema 即本端点 `SelectionDraft`，前端按 mode 映射 single / subset（含兼容别名 all））。
...
// Codex#7：sourceCandidateId / capabilityId / fromVersionId 必须【恰好三选一】（有且仅有一个）——
//          零个或多于一个（含 sourceCandidateId+capabilityId 这种两者并存）都拒，单条 .refine 计数判别。
export const CreateCapabilityBody = z
  .object({
    ...
    draftId: z.string().uuid().optional(), // 可选：回填该草稿的 version_id（续传衔接）；不计入「三选一」
  })
  .refine(
    (b) =>
...
// 注：draftId 是续传衔接的可选项，不参与「三个 source 恰好一个」判定
```
...
**行为**（三分支，按入参判别；Codex#7：先做「**恰好三选一**」入参校验——`sourceCandidateId`/`capabilityId`/`fromVersionId` 有且仅有一个，零个或多于一个均 `422 VALIDATION_FAILED`，再按那个唯一存在的字段进对应分支）：
...
| HTTP | code                   | retriable | action         | userMessage（人话）                                                                                                                              |
| ---- | ---------------------- | --------- | -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| 400  | `VALIDATION_FAILED`    | false     | `change_input` | 「输入有点问题：来源候选 / 已有能力体 / 被拒版本要恰好选一个（不能不选、也不能多选），改一下再试。」（Codex#7 恰好三选一：零个或多于一个均落此） |
...
```typescript
export const StartStructureBody = z
  .object({
    fields: z
      .array(
        z.enum(['name', 'tagline', 'role', 'goal', 'instructions', 'skill_set', 'starter_prompts']),
      )
...
```typescript
export const PatchManifestBody = z
  .object({
...
export const RegenerateFieldBody = z
  .object({
...
import type {
  ...
  FieldState, // 脊柱 §9
  JobStatus,
  DraftView, // 脊柱 §9（DraftView 为端点 G 响应类型）
} from '@/shared';
...
export interface PatchSelectionBody {
  selection: SelectionDraft;
}
...
export interface StartStructureBody {
  fields?: SoftFieldKey[];
}
export interface StartStructureResult {
  jobId: JobId;
...
export interface RegenerateFieldBody {
  reason?: 'stuck' | 'manual';
}
export interface RegenerateFieldResult {
  jobId: JobId;
...
export interface FieldStartPayload {
  field: SoftFieldKey;
  index: number;
  total: number;
}
export interface FieldDeltaPayload {
  field: SoftFieldKey;
  deltaText: string;
  itemIndex?: number;
}
export interface FieldDonePayload {
  field: SoftFieldKey;
  value: string | string[];
}
export interface FieldItemAppendedPayload {
  field: SoftFieldKey;
  itemIndex: number;
  value: string;
}
...
export interface StructureFieldFailedDetails {
  field: SoftFieldKey;
  attempts: number;
}
...
- **错误信封字段名 `userMessage` + 对外去 code**（脊柱 §11.B + D1）：各错误用例表「userMessage（人话）」列 = `userMessage`；**对外信封不含 code**，内部 code 仅日志/映射、经 traceId 关联。SSE `error` 帧 = 完整对外 `ErrorEnvelope`（Codex#2）。
...
- `state_snapshot`(首帧, structureState 全量) · `field_start` · `field_delta` · `field_done` · `item-appended`(数组逐条) · `progress`(「正在补全字段 4/7」) · `subtask` · `field_stuck`(三退路 continue/regen/wait) · `slow_hint` · `error`(STRUCTURE*FIELD_FAILED) · `done` · `heartbeat`。\*\*字段级帧 `field` 恒 SoftFieldKey，硬字段不发 `field*\*`/`field_stuck`/字段级 `error`。\*\*
