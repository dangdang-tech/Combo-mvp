...
import { makeLlmGateway } from '../infra/llm/gateway.js';
import { createMemoryAuditSink } from '../infra/llm/audit.js';
...
describe('OpenRouter.complete — 超时触发的 abort 归一为可重试超时（P1-3，不误判 CLIENT_CANCELLED）', () => {
  ...
  it('归一坐实：超时哨兵（LlmTimeoutError）→ retriable，而非 CLIENT_CANCELLED', async () => {
    ...
    const { normalizeLlmError } = await import('../infra/llm/errors.js');
    const { LlmTimeoutError } = await import('../infra/llm/gateway.js');
