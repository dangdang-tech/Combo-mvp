...
- **错误用例**（`userMessage` 列即脊柱 §11.B 唯一可展示人话；`code` 仅日志/映射、UI 不渲染）：
...
- **错误用例**（`userMessage` 列即脊柱 §11.B 的唯一可展示人话；`code` 仅日志/映射、UI 不渲染）：
...
```typescript
interface ImportedSegmentBrief {  // item-appended 的 item / progress.items[] 元素
  segmentId: Id;
...
> **拆表与 6.2 二选一**，默认采用 6.2 单表（简单、血缘直观）；拆表仅在存储压力大时启用。无论哪种，`candidate_evidence.segment_id`（提取域）指向「某 snapshot 下具体段」的语义不变（6.2 指 `session_segments.id`，拆表指 `snapshot_segments.id`），保证证据可回溯具体 snapshot、不跨快照（提取域契约对齐）。两方案均提供 §11.E 注册表的 `uq_session_segments_id_snapshot UNIQUE (id, snapshot_id)`（单表落在 `session_segments`、拆表落在 `snapshot_segments` 作可寻址段表），故 30 域复合 FK 的被引用键无论选哪种方案都存在、约束名一致。
...
```typescript
import type {
  Id, UserId, JobId, SnapshotId, DraftId, IsoDateTime,
  Envelope, Paginated, JobView,
} from '../shared';
...
export type RedactionCategory =
  | 'phone' | 'api_key' | 'email' | 'id_card' | 'bank_card' | 'ip' | 'secret_other';
