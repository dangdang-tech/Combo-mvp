...
>
> **真源**：产品行为以 `docs/开工总纲-创作者中心主链路.md` 第五/八章为准；验收口径以 `docs/测试验收-创作者中心主链路.md` 发布- / 选择结构化- / 主页- / 贯穿- 用例为准；技术架构以 `creator-builder/docs/01-详细技术方案.md` B-27~B-31 + §4 数据模型为准。
>
...
7. **写 outbox（同事务）**：`outbox_events(topic='capability.published', aggregate_id=capabilityId, payload={versionId, reviewStatus:'alpha_pending', visibility, ...})`，`xid` 取 `pg_current_xact_id()` 作提交序水位（B-13）。
...
> - **事务型 outbox**：`capability.published`/`capability.unpublished` 与业务写同 PG 事务，保证「发布成功 ⟺ 事件必投」，杜绝发了不上架 / 上架了没发。
...
const MarketCardPreviewBody = z.object({
  cover: z.object({                          // 与发布入参同形（可空 → 用默认字形图标，发布-25）
...
```typescript
const RetryBatchItemBody = z.object({
  cover: z.object({
    source: z.enum(['glyph', 'image', 'html_snapshot']),
    assetKey: z.string().optional(),
    snapshotRef: z.string().optional(),
...
- 两路都把被拒版的 `reject_reason` **镜像投影**到 `publications.reject_reason`（创作者侧可见态，发布页/工作台徽章/主页墙过滤三处经 `capability.review_resolved` 事件同步，发布-31）；权威原因仍以被拒版本行的 `reject_reason`/`rejected_at` 为准（§1.3）。
...
发布域写入 `outbox_events`（脊柱基表，结构见技术方案 §4），**与业务写同 PG 事务**，`xid xid8 DEFAULT pg_current_xact_id()` 作提交序水位。本域三类事件：

| topic | 何时写 | aggregate_id | payload 要点 | 消费方 |
|---|---|---|---|---|
| `capability.published` | 发布事务成功（§1.2）/ 评审 approve / 拒绝回退上一版 | capabilityId | `{ versionId, reviewStatus, visibility, slug, manifestHash }`（`slug` 取自 `capabilities.slug`，权威；投影 upsert listings 时 `listing.slug` 由 `trg_listing_slug` 强制对齐，不靠 payload，Codex#16） | MarketplaceProjection（upsert listings）、NotifyConsumer（通知创作者） |
| `capability.unpublished` | 评审拒绝且无上一版（下架，§2.6.1） | capabilityId | `{ versionId, reason:'review_rejected' }` | MarketplaceProjection（status→delisted 软删）、NotifyConsumer |
| `capability.review_resolved` | 评审裁决落定（approve/reject 均发，供工作台/主页同步） | capabilityId | `{ decision, reviewStatus, rejectReason?, currentVersionId, rejectedVersionId?, rejectedAt? }`（reject 时携被拒版本线信息，§1.3） | NotifyConsumer（创作者侧拒绝原因 + 重试/编辑提示，发布-31） |
...
```typescript
import type {
  Id, CapabilityId, VersionId, BatchId, CandidateId, JobId, Slug,
  JobStatus, ErrorEnvelope, Envelope,
} from '../shared';
...
export interface CreatePublishBatchBody { items: CreatePublishBatchItem[]; } // ≥1
...
| **B-30** | Alpha 人工评审 + 拒绝按可回退性分流（两线分明：被拒版本线 vs 当前对外版本线，Codex#8）+ 简单拒绝态落可见状态 | `POST /publications/{capId}/review`（§2.6.1）、`GET /publications/{capId}`（§2.6.2） | `publications`(review_status/reject_reason 镜像/current_version_id 回退)、`capability_versions`(被拒版 status/reject_reason/rejected_at + 上一版复位)、`outbox_events`(published/unpublished/review_resolved) | 发布-15/21/22/23/31、主页-19/23/24 |
| **B-31** | Auto-Eval 仅 schema/开关预留，不进本期发布路径 | （无端点；发布前不出现评测门） | `eval_reports`（仅冻结、本期不写） | 发布-02 改判（不出现自动评测门）、发布门覆盖反向验 |

> **跨域可见性（B-30 三处同步，发布-31）**：本域产出 `publications.review_status/reject_reason` + `capability.review_resolved` 事件；工作台域（F-05）读 review_rejected 徽章/原因 + 重试/编辑入口，主页域（B-33/F-06 作品墙）按评审结果过滤/回退展示，发布页（F-14）显示拒绝提示 + 重试/编辑。三处经同一 publications/事件真源，状态一致不滞后。
...
- 复用 `outbox_events`（capability.published / capability.unpublished / capability.review_resolved 三 topic）
