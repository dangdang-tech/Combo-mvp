...
编排在 `infra/docker-compose.yml`，14 服务 + 3 命名卷。固定启动顺序（硬性，由 `depends_on` + `condition` 落地）：
...
- 健康检查：postgres / redis×2 / minio 用原生探针；logto 断言 OIDC discovery（`{issuer}/.well-known/openid-configuration` 的 `issuer` / `jwks_uri`）；api 用 `/health`（liveness）。
