...
依赖什么：`platform/infra/logto.ts`（verifyLogtoJwt 验签）、`platform/infra/dev-session.ts`（dev 会话开关判定与验签）、`modules/account/repo.ts`（provisionUser，经 req.server.infra.db 写 PostgreSQL 的 users 表）、`platform/http/_helpers.ts`（sendError 错误信封）。间接触达 Logto 的 JWKS 端点和 users 表。
