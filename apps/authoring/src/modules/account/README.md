...
这个模块负责登录域：对接 Logto（外部登录服务，OIDC 协议）完成授权码登录，维护 cb_session 会话 Cookie，管理 users 表的首登建档，并提供当前用户信息接口。
...
3. 调 `platform/infra/logto-oidc.ts` 的 exchangeCodeForToken，用授权码加 code_verifier 向 Logto 换回 access_token 和 id_token。
...
6. 把 access_token 种进 cb_session Cookie（HttpOnly，8 小时），清掉短时事务 Cookie，302 回登录前记下的站内路径。
