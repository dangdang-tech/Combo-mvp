...
这个模块负责云端和本地提取结果的统一持久化、能力项查询与发布。能力项是提取流水线从对话历史里归纳出的可复用工作流，capabilities 表只存名称、摘要等轻量索引，完整可运行定义存在 MinIO 里（表里记对象键 storage_key）。
...
- `persist.ts` 是生产端唯一持久化入口：统一校验 `CapabilityDefinition v1`，生成或接受服务端预留的能力 ID，写固定对象键并插入 `capabilities` 轻量索引。Cloud Worker 与 Local Result API 都调用它。
- `index.ts` 是本域对外出口：业务域之间只能经它互引，向 task 域转出共享持久化服务和必要的能力查询。
...
被谁使用：路由由 `bootstrap/routes.ts` 挂载；`modules/task/pipeline.ts` 与 `modules/task/local-execution.ts` 在最终落库阶段经本域 `index.ts` 调共享持久化服务；`modules/task/repo.ts` 的任务视图 SQL 里子查询统计本表的能力项数。

依赖什么：`platform/http/_helpers.ts`（错误信封）、`platform/middleware/auth.ts`（登录守卫）、`platform/infra/db.ts`（Queryable 类型与 toIso 时间格式化）和 `platform/infra/object-store.ts`（对象存储端口）。外部资源包括 PostgreSQL 的 capabilities 表和 MinIO 的 combo-artifacts 桶；`persist.ts` 写定义对象，查询与发布接口只操作轻量索引。
