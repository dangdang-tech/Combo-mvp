...
describe('BullMQ 队列名禁含 ":"（live 崩溃修复）', () => {
  it('生产端 Queue：名无 ":"、命名空间走 prefix===QUEUE_PREFIX', async () => {
    const { createBullQueuePort, closeQueues } = await import('../platform/infra/queue.js');
  ...
  it('消费端 Worker：名无 ":"、命名空间走 prefix===QUEUE_PREFIX', async () => {
    ...
    vi.resetModules();
    vi.doMock('../platform/config/env.js', () => ({ loadEnv: () => env }));
    vi.doMock('../platform/infra/db.js', () => ({ getPool: () => ({}) }));
    vi.doMock('../platform/infra/redis.js', () => ({ getHotRedis: () => ({}) }));
    vi.doMock('../platform/sse/event-stream.js', () => ({ RedisEventStream: class {} }));
    vi.doMock('../platform/jobs/runner.js', () => ({ runJob: async () => 'ok' }));
    vi.doMock('../processes/register-handlers.js', () => ({})); // 阻断真实 handler 装配的副作用
    const importType: JobType = 'import';
    vi.doMock('../platform/jobs/registry.js', () => ({
