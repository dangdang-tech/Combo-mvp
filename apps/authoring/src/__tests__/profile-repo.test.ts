...
describe('readCreatorProfile — 六分区全量首屏（主页-01/02/03）', () => {
  ...
  it('③ 密度血缘用 current version source_candidate_id，不用公开能力 slug 反查候选 slug', async () => {
    const db = new ProfileFakeDb();
    const creatorId = seedProfile(db);
    const support = seedSupport(db, creatorId, 'md', [RECENT, OLD]);
    seedPublishedCapability(db, creatorId, {
      name: '文档与代码一致性核查',
      slug: 'cap-public-slug',
      sourceCandidateId: support.candidateId,
    });

    const res = await readCreatorProfile(db, creatorId, null, TODAY);
    const row = res!.profile.density.rows[0]!;
    expect(row.slug).toBe('cap-public-slug');
    expect(row.supportingSegments).toBe(2);
    expect(row.densityScore).toBe(100);
  });
  ...
  it('同一 source snapshot + candidate slug 的历史重复发布只保留最新公开代表', async () => {
    const db = new ProfileFakeDb();
    const creatorId = seedProfile(db);
    const source = seedSupport(db, creatorId, 'goal', [RECENT, OLD]);
    seedPublishedCapability(db, creatorId, {
      name: '融资材料深度审查',
      slug: 'cap-old-404',
    ...
    expect(res!.profile.metrics.capabilityCount).toBe(1);
    expect(res!.profile.density.rows.map((r) => r.slug)).toEqual(['cap-current']);
    expect(res!.profile.works.cards.map((c) => c.slug)).toEqual(['cap-current']);
    expect(serialized.includes('cap-old-404')).toBe(false);

    const works = await readWorksPage(db, creatorId, { limit: 24 }, TODAY);
    expect(works!.cards.map((c) => c.slug)).toEqual(['cap-current']);
  });
...
describe('owner 隔离：同 slug 跨创作者不串（密度段数 / 网络共现，主页-04/13/25/26）', () => {
  ...
  it('反向破坏：去掉 owner 限定（breakOwnerScope）→ 错指他人 source_candidate_id 时会串入 B 的段（测红，证明守门非空跑）', async () => {
    const db = new ProfileFakeDb();
    db.breakOwnerScope = true; // 反向破坏：跳过 source candidate 所属 snapshot 的 owner 校验。
    const aId = seedProfile(db, { user_id: 'creator-A3' });
    const bId = seedProfile(db, { user_id: 'creator-B3' });
    const bSupport = seedSupport(db, bId, 'b-source', [RECENT, RECENT, RECENT, RECENT, RECENT]);
    seedPublishedCapability(db, aId, {
      name: 'A 错指 B candidate 的能力',
      slug: 'a-public',
      sourceCandidateId: bSupport.candidateId,
    });
    const res = await readCreatorProfile(db, aId, null, TODAY);
    // 无 owner 限定 → A 的公开能力会读到 B candidate 的 5 段（与隔离期望 0 相反 → 断言能抓到越权回归）。
    expect(res!.profile.density.rows[0]!.supportingSegments).toBe(5);
  });
});
