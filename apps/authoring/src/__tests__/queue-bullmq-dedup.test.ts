...
//   Codex P0-2nd：假 Queue 的 add 同时【复刻真实 BullMQ custom jobId 校验】——含 ':' 即抛
//     `Custom Id cannot contain :`（两段 id split 长度 2 也崩）。旧 mock 不校验 opts.jobId 故漏掉了此 bug。
//     反向破坏：把 bullJobId 改回 ':' → add 抛错 → 本测转红（命中 live 崩）。
import { describe, it, expect, vi } from 'vitest';
import type { ReEnqueue } from '../platform/jobs/sweeper-reconcile.js';

/**
 * 复刻真实 BullMQ 5.78.1 custom jobId 校验（job.js validateOptions，约 1047 行）：
 *   jobId 含 ':' 且 split(':').length !== 3 → 抛 `Custom Id cannot contain :`。
 *   两段 id（`job-1:1`，split 长度 2）必崩——这正是 P0-2nd live 崩的根因。
 */
function assertValidBullCustomId(jobId: string): void {
...
class FakeBullQueue {
  ...
  async add(
    ...
    assertValidBullCustomId(opts.jobId); // 复刻真实 BullMQ custom jobId 校验（含 ':' 即崩）
...
const { createBullQueuePort, bullJobId } = await import('../platform/infra/queue.js');
const { reconcileJobsOnce, pgTypeLookup } = await import('../platform/jobs/sweeper-reconcile.js');

const env = { REDIS_QUEUE_URL: 'redis://localhost:6379/0' } as never;

describe('bullJobId 产出满足 BullMQ custom jobId 约束（Codex P0-2nd：无 ":"）', () => {
  it('两段触发 id 不含 ":"（含冒号两段 id 会被真实 BullMQ 拒，job 入队即崩）', () => {
    const id = bullJobId('job-1', 1);
    expect(id).not.toContain(':'); // 反向破坏：改回 ':' → 此断言转红
    expect(() => assertValidBullCustomId(id)).not.toThrow(); // 复刻真实校验：不抛
  });

  it('jobId 自身含连字符也不引入 ":"（UUID/ULID 形态安全）', () => {
    const id = bullJobId('01HXXXXXX-abc' as never, 12);
    expect(id).not.toContain(':');
    expect(() => assertValidBullCustomId(id)).not.toThrow();
  });

  it('对照：旧的冒号写法会被真实 BullMQ 校验拒（证明 mock 能抓到 bug）', () => {
    // 复刻旧 bullJobId 产出 `job-1:1`（两段、含冒号）→ 真实 BullMQ 抛 `Custom Id cannot contain :`。
    expect(() => assertValidBullCustomId('job-1:1')).toThrow('Custom Id cannot contain :');
  });
});
