// 事件管道共用的最小 DB 抽象（70 域，B-13~B-16/B-35）。
//   - QueryableDb：仅依赖 `query` 的 pg.Pool/Client 子集（与 users-repo 同口径），便于 mock 单测。
//   - Tx：单连接事务句柄（BEGIN/COMMIT/ROLLBACK 由 withTransaction 收口）。
//   - withTransaction：从池领连接、开事务、回调内复用同一连接（outbox 与业务同事务、
//     cursor 与处理同事务的硬约束都靠它，70 §2.1/§3.3）。
// 无 PG 也能跑：单测注入 mock Queryable/Pool。
