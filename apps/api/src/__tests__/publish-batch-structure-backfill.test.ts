...
import { structureCandidateItem, type OnVersionCreatedInTx } from '../publish/batch-structure.js';
import { createCapability } from '../structure/create-capability.js';
...
describe('B-29 批编排「已生成不丢 + 重试不重复建版」· create+回填同事务原子（r7 原子窗口守门）', () => {
  ...
  it('反向破坏（create 与回填非原子 且 create 无幂等：旧实现）→ create 先 COMMIT 但回填丢失 → 重试重复建版（versions.size===2）——守门断言「不应重复」会红', async () => {
    const { db, tx, gateway } = setup();
    seedCandidateWithEvidence(db, CAND, OWNER);
    startBatchJob(db, JOB, OWNER, 1);
    const item: FakeItemRow = { version_id: null, capability_id: null };

    // 复现旧实现的【非原子 + 无幂等】路径：create-capability 先【独立事务 COMMIT】建版（版本已落库），但回填【未发生】（item.version_id 仍 null）。
    //   这正是 r7 原子窗口的坏后果：create 后回填前接管 → 版本残留、item 无指针。
    ...
    // 重试：item 无 versionId → 不带 existingVersionId → create-capability 据 candidate 再建一版（无幂等键回放）→ 重复建版。
    const retry = await structureCandidateItem(
      { db, txPool: tx, gateway },
      {
        candidateId: CAND,
    ...
    expect(retry.kind).toBe('ready');
    if (retry.kind !== 'ready') return;
    // 守门：非原子 + 无幂等下【重复建版】（第二个版本行，且 ≠ 第一遍版本）——若改回原子方案则第一遍不残留、重试不重复（versions.size===1）。
    expect(db.versions.size).toBe(2); // ← 缺口坏行为：建了两版。
    expect(retry.versionId).not.toBe(created1.versionId);
  });
});
