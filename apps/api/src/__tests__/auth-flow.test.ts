...
import type * as LogtoOidcModule from '../infra/logto-oidc.js';
...
vi.mock('../infra/logto-oidc.js', async () => {
  const actual = await vi.importActual<typeof LogtoOidcModule>('../infra/logto-oidc.js');
  return {
...
const { loginHandler, callbackHandler, logoutHandler, meHandler, AUTH_TX_COOKIE } =
  await import('../routes/auth-handlers.js');
const { SESSION_COOKIE } = await import('../middleware/auth.js');
...
describe('cb_session 会话层贯穿（10-auth §2/§4 / §5）', () => {
  ...
  it('cb_session cookie 被 requireSseAuth 识别（同源 Cookie 鉴权，禁 Bearer/query）', async () => {
    const { requireSseAuth } = await import('../middleware/auth.js');
    verifyMock.mockResolvedValue({
      kind: 'ok',
