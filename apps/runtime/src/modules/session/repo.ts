...
export async function createSession(
  ...
  const res = await pool.query<SessionDbRow>(
    `INSERT INTO rt_chat_sessions
       (owner_id, capability_id, slug, version, title, instructions, manifest_hash, public_view)
...
export async function listSessions(
  ...
  const res = await pool.query<{
    ...
    `SELECT id, slug, title, COALESCE(public_view ->> 'name', '') AS capability_name, updated_at
...
export async function getMessages(pool: Pool, sessionId: string): Promise<RuntimeMessage[]> {
  const res = await pool.query<{
    ...
    `SELECT id, seq, role, text, artifacts, created_at
...
export async function saveTurn(pool: Pool, input: SaveTurnInput): Promise<SaveTurnResult> {
  ...
  try {
    ...
    const u = await client.query<{ created_at: Date }>(
      `INSERT INTO rt_chat_messages (id, session_id, seq, role, text, artifacts)
       VALUES ($1, $2, $3, 'user', $4, '[]'::jsonb)
       RETURNING created_at`,
      [input.user.id, input.sessionId, userSeq, input.user.text],
    );
