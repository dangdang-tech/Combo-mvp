...
export function createEventLogEmitter(input: EventLogEmitterInput): AguiEmitter {
  ...
  return {
    ...
    textStart: (messageId) =>
      enqueue({ type: EventType.TEXT_MESSAGE_START, threadId: input.threadId, runId: input.runId, messageId, role: 'assistant' }),
    textContent: (messageId, delta) =>
      enqueue({ type: EventType.TEXT_MESSAGE_CONTENT, threadId: input.threadId, runId: input.runId, messageId, delta }),
    textEnd: (messageId) =>
      enqueue({ type: EventType.TEXT_MESSAGE_END, threadId: input.threadId, runId: input.runId, messageId }),
    stateDelta: (ops) =>
      enqueue({ type: EventType.STATE_DELTA, threadId: input.threadId, runId: input.runId, delta: ops }),
    stateSnapshot: (snapshot) =>
      enqueue({ type: EventType.STATE_SNAPSHOT, threadId: input.threadId, runId: input.runId, snapshot }),
    runError: (message) =>
